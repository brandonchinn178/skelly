...
data Service = Service
  { getPackageDeps :: PackageId -> IO PackageDeps
  , getPackageVersions :: PackageName -> IO [Version]
  }

initService :: PackageIndex.Service -> Service
initService pkgIndexService =
  Service
    { getPackageDeps = PackageIndex.getPackageDeps pkgIndexService
    , getPackageVersions = PackageIndex.getPackageVersions pkgIndexService
    }

-- | Get the list of transitive dependencies in topological order.
run :: Service -> PackageDeps -> IO [PackageId]
run Service{..} deps0 =
  runMaybeT (go (compileDeps deps0) (Map.keys deps0))
    ...
    go deps (pkgName : rest) =
      case Map.Strict.lookup pkgName deps of
        ...
        Just range
          ...
          | isSingletonRange range -> go deps rest
          -- package not resolved
          | otherwise-> do
              versions <- liftIO $ getVersionPreferences range <$> getPackageVersions pkgName
              ...
              asum
                [ do
                    ...
                    pkgDeps <- liftIO $ getPackageDeps pkgId
                    ...
                    (pkgId :) <$> go (Map.Strict.insert pkgName (singletonRange pkgVer) deps') rest
                | pkgVer <- versions
