...
spec = do
  ...
  -- TODO: returns topological

  -- TODO: uncomment after topological
  -- prop "efficiently backtracks" $ do
  --   -- project => A, B
  --   -- A (2.1, 2.2, ..., 1.0, 2.10, 2.11, ...) => []
  --   -- B (1.0) => A ^1.0
  --   nameA <- forAll $ Gen.text (Range.linear 1 20) Gen.unicode
  --   nameB <- forAll $ Gen.text (Range.linear 1 20) Gen.unicode
  --   when (nameA == nameB) discard
...
toPackageId s =
  case Text.splitOn "-" s of
    [name, versionStr] | Just version <- parseVersion versionStr -> PackageId name version
